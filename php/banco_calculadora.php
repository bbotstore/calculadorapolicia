<?php
    ...
    if($_REQUEST['tab'] == 'calculadora'){
        if($_REQUEST['acao'] == 'lista_usuarios'){
			...
			$sql = "INSERT INTO prisao (primeiro_oficial, segundo_oficial, terceiro_oficial, quarto_oficial, quinto_oficial, sexto_oficial, setimo_oficial, oitavo_oficial, nome_detento, id_detento, itens_prisao, artigos_prisao, pena_prisao, multa_prisao, fianca_prisao, reu_primario, reu_confesso, adv_constituido, qsj_prisao, obs, dt_prisao, bombeiro, autenticacao, observacoes) VALUES (:primeiro_oficial, :segundo_oficial, :terceiro_oficial, :quarto_oficial, :quinto_oficial, :sexto_oficial, :setimo_oficial, :oitavo_oficial, :nome_detento, :id_detento, :itens_prisao, :artigos_prisao, :pena_prisao, :multa_prisao, :fianca_prisao, :reu_primario, :reu_confesso, :adv_const, :qsj_prisao, :obs, :dt_prisao, :bombeiro, :autenticacao, :observacoes)";
			$cmd = $con->prepare($sql);
			...
			$cmd->bindValue(':observacoes', $obs_);
			if($cmd->execute()){
				echo json_encode('ok');
				$data_registro = date('Y-m-d H:i:s');
				envia_apreensao($primeiro_oficial,$segundo_oficial,$terceiro_oficial,$quarto_oficial,$quinto_oficial,$sexto_oficial,$setimo_oficial,$oitavo_oficial,$nome_detento,$id_detento,$artigos,$itens,$pena,$fianca,$multa,$primario,$confesso,$adv,$id_registro,$qsj_,$data_registro,$nome_bombeiro_,$autenticacao,$obs_);
			} else {
