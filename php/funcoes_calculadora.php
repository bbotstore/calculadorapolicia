<?php
    ...
    if($_REQUEST['tab'] == 'calculadora'){
        if($_REQUEST['acao'] == 'lista_usuarios'){
			...
			$sql = "SELECT nome_discord_usuario,id_discord_usuario FROM usuarios WHERE guarnicao = '- Polícia Federal' OR guarnicao = '- P.R.F' OR guarnicao = '- P.M.E.R.J' OR guarnicao = '- P.C.E.R.J'";
            $cmd = $con->prepare($sql);
            $cmd->execute();
            $dados = [];
